...
public class XMLProducer {
	public static String CONFIGURE_TIMEOUT = "__timeout";
	public static String CONFIGURE_ISASYNC = "__isAsync";
	...
	HashSet <String> objectRedefinedParent = new HashSet<String>();
...
//	}
	...
	protected void addParents(Element obj, DObject o, String info) {
		boolean print = false;
		if (o.getParents().size()==0) return;
		Element parent = obj.addElement("parent");
		String [] hItems = Configuration.getHierarchy(o.getFname());
		if (hItems!=null) {
			String hP = hItems[2];
			if (!hP.equals("ne")) {
				System.out.format("Don't support parent type:%1$-20s for object:%2$s%n",hP,o.getFname());
				System.exit(2);
			}
			if (print) System.out.println("\tredefine partent for object "+o.getFname() + " " + info);
			this.objectRedefinedParent.add(o.getFname());
	...
	protected void addParents(Element obj, DObject o, HashSet<String> parents,String info) {
		addParents(obj, o, parents,true, info);
	}
	
	protected void addParents(Element obj, DObject o, HashSet<String> parentS, boolean addSubslot,String info) {
	...
	protected void addBasicType(Element attributeE, String[] typeItem, String limit) {
		addBasicType(attributeE,typeItem,limit,true);
	}
	
	protected void addBasicType(Element attributeE, String[] typeItem, String limit, boolean vclosedbyRule) {
		...
		if (ctype.equals("integer")) {
			...
			if (vclosedbyRule) attributeE.addAttribute(Configuration.value_closed_by, "1");
...
//		if (Configuration.getGetOnlyAttribute(o.getFname()).size()>0) return true; 
	...
	protected void addMetadata(DObject o, Element element, String type, String metaInfo) {
		if(metaInfo!=null) {//only from call method so far
			if (!metaInfo.trim().isEmpty()) {
				...
				for (String metaItem:metaItemA) {
					...
					if (nameValuePairA.length!=2) {
						System.out.println("Invalid meta define for " + type + " " + element.attributeValue("name")+" of object "+o.getFname());
						System.exit(2);					
					}
					element.addAttribute(nameValuePairA[0], nameValuePairA[1]);
				}
			}
		}		
	}
	...
	protected Element addAttribute(Element actionE, DObject o, String name, String type, String limit,String metaInfo) {//so far metaInfo only for call 
		...
		boolean vclosedbyRule = true;
		if(metaInfo!=null) {//from call method, we ignore general vclosedby rule
			vclosedbyRule = false;
			addMetadata(o, attributeE, "attribute", metaInfo);
//			if (!metaInfo.trim().isEmpty()) {
//				String [] metaItemA = metaInfo.split(",");
//				for (String metaItem:metaItemA) {
//					String[] nameValuePairA= metaItem.split("=");
...
//			}
		...
		if (referredObjs != null) {
			...
			if (type.equals("TXT") && vclosedbyRule) attributeE.addAttribute(Configuration.value_closed_by, "1");
...
//				System.out.println("qos in object "+ o.getFname());
			...
			if (type.equals("VPNNAME")) {
				...
				addBasicType(attributeE,ti,limit,vclosedbyRule);
			}
...
//		addExtraGetAttributesInOtherAPI(actionE,o);
	...
	protected void addSetAction(Element obj,DObject o){
		...
		if (Configuration.isActionToAddTimeout(o.getFname(), "set")) {
			this.addAttribute(actionE, o, CONFIGURE_TIMEOUT, "U32", "11s-"+Configuration.timeout+"s");
		}
	}
	...
	protected void clearObjectMetaInfo() {
	...
	private void addHierarchyMetaInfo(DObject o, String string) {
		String [] item = Configuration.getHierarchy(o.getFname());
		if (item !=null) {
			objectMetaInfo.put("hierarchy", item[1]);
		}
	}
	
	protected void addMetaInfo(TreeMap<String,String> metaData) {
		for (Entry<String,String> item:metaData.entrySet()) {
			objectMetaInfo.put(item.getKey(), item.getValue());
		}
	}
	...
	protected void addPhysicalInterface(Element platform, DObject o) {
		...
		for (String sn:sns) {
			...
			this.addParents(obj, o, parentS, addSubslot,"");
		}
	}
	
	protected void addSubslotElement(Element platform, DObject o) {
		Element obj = platform.addElement("object");
	...
	protected void addEnvironmentObject(Element platformE) {
		Element envE = platformE.addElement("object");
		envE.addAttribute("name", "environment");
		envE.addAttribute("address", "0");
		Element actionE = envE.addElement("action");
		actionE.addAttribute("type","stl");
		actionE.addAttribute("name","set");
		String [][] actionDefs = {
			{"set","platform-name","string","",""},
			{"set","platform-value","string","",""},
			{"set","build-name","string","",""},
			{"set","build-number","string","",""},
			{"set","release-name","string","",""},
			{"set","release-number","string","",""},
			{"set","suppress-manual","enum","yes/no",""},
			{"set","break-upon","enum","verify-fail/op-fail/both/off",""},
			{"set","cli-display-mode","enum","yes/no",""},
			{"set","stub-mode","enum","off/normal/raw",""},
			{"set","simplified-log","enum","yes/no",""},
			{"set","stopping-criterion","enum","stop-test-upon-failure/stop-case-upon-failure/stop-test-upon-verify-failure/stop-case-upon-verify-failure/none",""}
		};
		for (String [] act:actionDefs) {
			this.addFunctionAttribute(actionE, act);
		}
		actionE = envE.addElement("action");
		actionE.addAttribute("type","stl");
		actionE.addAttribute("name","set_summary_variables");
		String [][] actionDefs1 = {
			{"set_summary_variables","variable_name","string","",""},
			{"set_summary_variables","variable_value","string","",""}
		};
		for (String [] act:actionDefs1) {
			this.addFunctionAttribute(actionE, act);
		}
		actionE = envE.addElement("action");
		actionE.addAttribute("type","stl");
		actionE.addAttribute("name","log");
		String [][] actionDefs2 = {
			{"log","type","string","",""},
			{"log","message","string","",""}
		};
		for (String [] act:actionDefs2) {
			this.addFunctionAttribute(actionE, act);
		}	
	}

	protected void addNEOtherActions(Element objE, DObject o) {
		Element actionE = objE.addElement("action");
		actionE.addAttribute("type","cli");
		actionE.addAttribute("name","disconnect");
		actionE.addAttribute("acttype","internal");
		actionE.addAttribute("actfunc","6");
		...
		actionE = objE.addElement("action");
		actionE.addAttribute("type","cli");
		actionE.addAttribute("name","popDataFromDriver");
		actionE.addAttribute("acttype","2");
		actionE.addAttribute("actfunc","4");
		String [][] actionDefs = {
			{"popDataFromDriver","resp","","",""},
			{"popDataFromDriver","__timeout","integer","11-"+Configuration.timeout+"s",""},
			{"popDataFromDriver","__prompt","string","",""},
			{"popDataFromDriver","__poundNumber","integer","1..4",""}
		};
		for (String [] act:actionDefs) {
			this.addFunctionAttribute(actionE, act);
		}
	...
	protected void addSetContextAction(Element objE, DObject o) {
		...
		actionE.addAttribute("acttype", "internal");
		actionE.addAttribute("actfunc", "1");
		actionE.addAttribute("isNode", "true");
		addAttributes(actionE, attributes);
		Element attrE = actionE.addElement("attribute");
		attrE.addAttribute("name", "__nameValuePair");
		Element value = attrE.addElement("value");
		value.addAttribute("type", "format");
		value.addAttribute("format", "name::value");
	}
	...
	protected void addObject(Element platform, DObject o) {
		...
		case DObject.OBJECT_ASSOCIATION_TYPE_OPOREF:
			...
			if (objectMapper.get(o.getFname()).equals("slot")){//hard code for slot object
				...
				addSubslotElement(platform, o); //hard code for subslot object
			}
			as = o.getAssociation(o.getParents().firstElement().getFname());
			addParents(obj,o, "OPOREF");			
			if (as.contains("[S]")) {
...
//					System.out.println("add metainfo for OPOREF S object "+o.getFname());	
				...
				addHierarchyMetaInfo(o,"OPOREF:S");
				addStaticObjectMetaInfo(obj);
...
//			obj.addAttribute("auto-api-mode", "opmr");
			...
			int type = Configuration.getAssTypeForOPMR(as);
			this.clearObjectMetaInfo();
			String detailedType = Configuration.getTypeString(type);
			obj.addAttribute("auto-api-mode", "opmr"+":"+detailedType);
			switch (type) {
			case Configuration.ALL_DYNAMIC:
				...
				addHierarchyMetaInfo(o, "OPMREF:ALL_DYNAMIC");
				addStaticObjectMetaInfo(obj);
				addParents(obj,o,"OPMREF:ALL_DYNAMIC");			
				if (!Configuration.isObjectWithoutCreate(o.getFname())) {
//					TreeMap<String, Vector<DTableItem>> assObjs = o.getParentAssociationObject();
...
//				addSetAction(obj,o);
			...
			case Configuration.ALL_STATIC: //we create an abstract object, all other static object inherits it, we assume they are all fixed
				...
				for (String a:aL) {
					...
					refreshStaticObjectMetaInfo(key);
					addHierarchyMetaInfo(o, "OPMREF:ALL_STATIC");					
					addStaticObjectMetaInfo(obj1);
					addParents(obj1,o,"OPMREF:ALL_STATIC");
				}
				if (subNodeNames.size()>0) {
					addDecomposedObject(o,subNodeNames);
				} else {
		...
		case DObject.OBJECT_ASSOCIATION_TYPE_MPOREF:
			...
			if (Configuration.getHierarchy(o.getFname())!=null) {
				System.out.println("Unexception hierarchy definition for MPOREF object "+o.getFname());
				System.exit(2);
			}
			pObj = o.getParents();
	...
	protected void addCallActions(Element obj, DObject o) {
		...
		for (Entry<String,DObjectAction>a:aS) {
			...
			String [] actMetadata = Configuration.getCallMetaData(o.getFname(), aName);
			if (actMetadata != null) {
				addMetadata(o, actionE, "action", actMetadata[2]);
			}
			...
			this.addAttribute(actionE, o, CONFIGURE_TIMEOUT, "U32", "11s-"+Configuration.timeout+"s");
			if (env==null || env.trim().isEmpty()) {
				String [] attr = {"call-"+aName, CONFIGURE_ISASYNC,"enum","yes/no",""};
				this.addFunctionAttribute(actionE, attr);
			}
	...
	public void addSubClassObject(Element platform, DObject o, String as, HashSet <String>parents, 
		...
		if (as.contains("[S]")) {
			...
			addHierarchyMetaInfo(o, "addSubClassObject:S");
			addStaticObjectMetaInfo(obj);
			this.addParents(obj, o, parents,"addSubClassObject:S");
			if (as.contains("[F0]")) { //it is not fixed, need create method
				addCreateAction(obj,o,as);
			}
		} else if (as.contains("[D]")) {
			if (!Configuration.isObjectWithoutCreate(o.getFname())) {
				...
				addHierarchyMetaInfo(o, "addSubClassObject:S");
				addStaticObjectMetaInfo(obj);
				this.addParents(obj, o, parents,"addSubClassObject:D");
				addDynCreateAction(obj,o,as);
			}			
		} else {
...
//		});
		...
		if (!Configuration.withFolder) {
			Configuration.checkDuplicated(Configuration.hierarchyObjects);
			if (this.objectRedefinedParent.size()!=Configuration.hierarchyObjects.length) {
				boolean found = false;
				for (String [] item:Configuration.hierarchyObjects) {
					if (!this.objectRedefinedParent.contains(item[0])) {
						System.out.println("the object "+item[0]+" is not in redefined list");
						found = true;
					}
				}
				if (found) {
		...
		System.out.println("There are "+Configuration.actionTotal+"["+Configuration.actionParsed+"] actions.");
	}
	
	protected void printObjectHierarchyChanged() {
		boolean first = true;
		System.out.format("\t%1$-40s -- %2$s%n","Object Name in CATS", "Name in API Doc");
		System.out.println("================================================================================================");
		for (String item:objectRedefinedParent) {
			String [] subObjs = Configuration.getObjectDecomposed(item);
			if (subObjs == null) {
				System.out.format("\t%2$s%1$-40s -- %3$s%n",this.objectMapper.get(item), Configuration.name_prefix,item);
			} else {
				first = true;
				for (String obj:subObjs) {
					if (first)
						System.out.format("\t%2$s%1$-40s -- %3$s%n",obj, Configuration.name_prefix,item);
					else
						System.out.format("\t%2$s%1$-40s%n",obj, Configuration.name_prefix);
					first =false;
				}
	...
	public void addProperties(Element platform) {
		...
		Element propertyE = platform.addElement("property");
		propertyE.addAttribute("name", "modelRevision");
		propertyE.addAttribute("value", Configuration.xml_revision);
		Element productE = platform.addElement("product");
		productE.addAttribute("name", "7090M");
		productE.addAttribute("release","3.0");
		productE = platform.addElement("product");
		productE.addAttribute("name", "7090M");
		productE.addAttribute("release","3.0.10");
	}
	...
	public void addFunctionAttribute(Element funcE, String[] attr) {
		...
		String type = attr[2];
		if (CommonUtils.isNullOrSpace(type)) return;
		Element valueE = attrE.addElement("value");
		valueE.addAttribute("type", type);
		if (type.equals("integer") || type.equals("format")) {
			valueE.addAttribute("range", attr[3]);
		} else if(type.equals("enum")) {
			String [] values = attr[3].split("/");
			for(String item:values) {
				Element option = valueE.addElement("option");
				option.addAttribute("name", item);
			}
		}
	}
...
//			addEnvironmentObject(platform);
			...
			String fileName = "7090MCli.xml";
			if (Configuration.withFolder) fileName = "7090MCli-with-folder.xml";
			XMLWriter writer = new XMLWriter(new FileWriter("./model/"+fileName),format);
			writer.write(doc);
			writer.close();
		} catch (Exception e) {
			e.printStackTrace();
